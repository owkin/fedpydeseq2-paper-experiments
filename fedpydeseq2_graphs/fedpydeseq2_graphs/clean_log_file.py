...
from fedpydeseq2_graphs.utils.utils import remove_shape_tags
...
def filter_nested_remotes(input_text: str) -> str:
    ...
    nested_pattern = re.compile(
        r"(<remote_data>(?:(?!<\/?remote_data>).)*<remote_data>(?:(?!<\/?remote_data>).)*<\/remote_data>(?:(?!<\/?remote_data>).)*<\/remote_data>)",
        re.DOTALL,
    )
    ...
    while True:
        ...
        inner_pattern = re.compile(
            r"<remote_data>(?:(?!<\/?remote_data>).)*<\/remote_data>", re.DOTALL
        )
...
def merge_iterations(input_text: str) -> str:
    """Merge consecutive <iteration> blocks with the same content.

    Combine consecutive <iteration> blocks with the same content into a single block.
    The <number> tags are merged into a range of numbers.
...
def process_log_file(input_path: pathlib.Path, output_path: pathlib.Path) -> None:
    """Process a log file by filtering remote_data blocks and merging blocks.
...
def main():
    ...
    This function loads the paths from a YAML file, processes the raw workflow log file
    by filtering out duplicate <remote_data> blocks and merging consecutive <iteration>
    blocks, and writes the cleaned log file to the specified output path.
    """
