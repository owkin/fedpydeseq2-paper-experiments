...
def get_first_last_shared_state_id_from_block(
    block: FunctionBlock | LeafFunctionBlock, max_depth: int | None
) -> tuple[int, int] | None:
    """Retrieve the first and last shared state IDs.

    This is done from a function block.
...
def get_function_block_by_name(
    ...
    """Recursively search for a FunctionBlock by name.
