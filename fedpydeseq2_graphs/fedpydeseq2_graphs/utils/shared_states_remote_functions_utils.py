...
from fedpydeseq2_graphs.utils.data_classes import Item
from fedpydeseq2_graphs.utils.data_classes import RemoteFunctionRpz
...
def process_input_output_blocks(input_text: str) -> tuple[str, dict[int, SharedState]]:
    """Process blocks in the given text and replace them with unique IDs.
    ...
    assert len(input_matches) == len(output_matches), (
...
def update_items_from_functions(
    ...
    for i, remote_function in enumerate(remote_functions):
        assert remote_function.input_id in shared_states, (
        ...
        assert remote_function.output_id in shared_states, (
...
def create_shared_states_remote_functions(
    ...
    """Create SharedState and RemoteFunctionRpz from the given text.
